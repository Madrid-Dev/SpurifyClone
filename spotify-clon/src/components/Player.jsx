...
const VolumeControl = () =>{
    ...
    return (
    <div className='flex justiy-center gap-2'>
            {volume < 0.1 ? <button onClick ={handleClicktoVolume} ><VolumeSilence/></button>  : <button onClick ={handleClicktoSilence}> <Volume  /> </button>}
...
const SongControl = ({audio}) =>{
    ...
    const formatTime = time =>{
        ...
        //PadStart es una funcion para rellenar los strings. Pongo cuanto tiene que medir minimo el string
        //Si no mide eso entonces se pone, el caracter de parametro en los lugares faltantes
    ...
    return(
        <div className='flex gap-2'>
            <span className="opacity-70 text-right w-12">{formatTime(currentTime)}</span>
            ...
            {duration ? <span className='opacity-70 w-12'>{formatTime(duration)}</span> : '0:00'}
        </div>
    )
}
...
export function Player () {
    const { isPlaying, setIsPlaying, currentMusic, volume } = usePlayerStore(state => state) //Loading global state
